...
def ask(question, message):
    ...
    while len(chat_logs[channel_id]["content"]) > 2000:
        array = chat_logs[channel_id].split("\n")
        array.pop(0)
        chat_logs[channel_id] = "\n".join(array)
...
async def on_message(message):
    if message.author != bot.user:
        if bot.user.mentioned_in(message) or message.channel.id == 735091182769537037:
